...
class Authorizer {
    ...
    authorize(event) {
        ...
        let tmp = authorizationHeader.split(' ');   // Split on a space, the original auth looks like  "Basic Y2hhcmxlczoxMjM0NQ==" and we need the 2nd part
        let buf = Buffer.from(tmp[1], 'base64'); // create a buffer and tell it the data coming in is base64
        let plainAuth = buf.toString(); // read it back out as a string
